...
export interface Theme {
  ...
  /** Section-space scale (multiples of `spacing`) — drives `py-section-*` etc. */
  sectionSpace?: Record<string, string>;
  /** Heading scale (`h1..h6` + `x-to-y` pairs) — drives `h1..h6` utilities. */
  ...
  // Plural keys match preset-mini's `grid-cols-*` / `grid-rows-*` theme
  // lookup (`theme.gridTemplateColumns[v]` etc.), so templates defined
  // here work with both our preset's `grid-{k}` rule and preset-mini's
  // native grid utilities if ever reused.
